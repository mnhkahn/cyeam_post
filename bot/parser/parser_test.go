...
// 	p := NormalParser{}
// 	host, err := p.GetHost("http://blog.cyeam.com/abc")
// 	fmt.Println(host, err)
// }

// func TestCyParser(t *testing.T) {
// 	p := RegParser{}
// 	urls := p.GetAs(body, "http://blog.cyeam.com")
// 	fmt.Println(urls)
// }

func TestMainBody(t *testing.T) {
	// p := NormalParser{}
	p, _ := NewCssParser(strings.NewReader(body_post))
	reg := RegParser{}
	// main := p.GetMainBody(reg.RemoveHtml(body_post))
	body := ""
	res := p.GetChildren("body")
	for _, r := range res {
		temp := reg.RemoveHtml(r)
		if len(temp) > len(body) {
			body = temp
		}
	}
	fmt.Println(body)
...
-->  
      ...
      <footer>
		...
		<a rel="license" href="http://creativecommons.org/licenses/by/4.0/"><img alt="Creative Commons License" style="border-width:0" src="http://i.creativecommons.org/l/by/4.0/88x31.png" /></a><br /><span xmlns:dct="http://purl.org/dc/terms/" href="http://purl.org/dc/dcmitype/Dataset" property="dct:title" rel="dct:type">Cyeam</span> by <a xmlns:cc="http://creativecommons.org/ns#" href="www.cyeam.com#About" property="cc:attributionName" rel="cc:attributionURL">Bryce</a> is licensed under a <a rel="license" href="http://creativecommons.org/licenses/by/4.0/">Creative Commons Attribution 4.0 International License</a>.
